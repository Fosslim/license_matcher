...
describe LicenseMatcher::TFRubyMatcher do
  ...
  it "finds correct matches for text files" do
    expect( lm.match_text(mit_txt).label ).to eq("MIT")
    expect( lm.match_text(pg_txt).label ).to eq('PostgreSQL')
    expect( lm.match_text(lgpl_txt).label ).to eq('LGPL-2.0')
    expect( lm.match_text(pg_txt).label ).to eq('PostgreSQL')
    expect( lm.match_text(bsd3_txt).label ).to eq('BSD-3-Clause')
  end
  ...
  it "finds correct matches for html files" do
    ...
    expect( lm.match_html(mit_html, 0.5).label ).to eq('MIT')
    expect( lm.match_html(apache_html).label ).to eq('Apache-2.0')
    expect( lm.match_html(bsd3_html, 0.0).label.downcase ).to eq('bsd-4-clause-uc')

    #how it handles noisy pages
    spdx_id = lm.match_html(apache_aws, 0.5).label
    expect( spdx_id ).to eq('Apache-2.0')

    spdx_id = lm.match_html(apache_plex, 0.0).label
    expect( spdx_id ).to eq('Apache-2.0')

    spdx_id = lm.match_html(bsd_fparsec, 0.0).label
    expect( spdx_id ).to eq('BSD-2-Clause')

    spdx_id = lm.match_html(mit_ooi, 0.0).label
    expect( spdx_id ).to eq('MIT')

    spdx_id = lm.match_html(mit_bb, 0.0).label
    expect( spdx_id ).to eq('MIT')

    expect( lm.match_html(mspl_ooi, 0.0).label ).to eq('MS-PL')

    spdx_id = lm.match_html(cpol, 0.0).label
